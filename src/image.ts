...
export async function renderFrameToOgImage(
  ...
  // Calculate dimensions - use minimal padding to maximize content area
  const lineHeightPx = Math.round(fontSize * lineHeight)
  const paddingY = 20
  const paddingX = 24

  // Calculate effective line height accounting for overlap
  // (lineToNode uses negative margin of ~50% of extra space)
  const gapOverlap = Math.round((lineHeight - 1) * fontSize * 0.5)
  const effectiveLineHeight = lineHeightPx - gapOverlap

  // Calculate how many lines fit in the fixed height
